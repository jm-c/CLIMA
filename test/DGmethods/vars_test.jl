...
vars_state(::VarsTestModel, T) = @vars(x::T,
                                       coord::SVector{3, T})
vars_aux(m::VarsTestModel, T) = @vars(coord::SVector{3, T},
                                      polynomial::T)
...
function init_state!(m::VarsTestModel, state::Vars, aux::Vars,
                     coord, t::Real)
  @inbounds state.x = coord[1]
  state.coord = coord
end

function init_aux!(::VarsTestModel{dim},
                   aux::Vars,
