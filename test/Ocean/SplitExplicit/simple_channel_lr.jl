...
struct ReentrantChannel{T, BC} <: AbstractOceanProblem
    Lˣ::T
    Lʸ::T
    H::T
...
function ocean_init_state!(p::ReentrantChannel, Q, A, localgeo, t)
    ...
    #- add small random noise to Initial Temp. field
    Random.seed!(0)
    Q.θ += rand(FT) * 1.e-3
    #- not sure why the following does not work:
    # Q.θ += rand(MersenneTwister(0), FT, size(Q.θ)) * 1.e-3
...
function main(; restart = 0)
    ...
    BC = (
        ClimateMachine.Ocean.SplitExplicit01.CoastlineFreeSlip(),
        ClimateMachine.Ocean.SplitExplicit01.OceanFloorLinearDrag(),
        ClimateMachine.Ocean.SplitExplicit01.OceanSurfaceStressForcing(),
    )
    prob = ReentrantChannel{FT, typeof(BC)}(
        Lˣ,
        Lʸ,
        H,
        BC,
    ...
    #- set model time-step:
    dt_fast = 5      # should be stable with dt = 9.6 s
    dt_slow = 120
    if t_chkp > 0
...
vtkpath = "vtk_split"
 ...
 const runTime = 360 * 86400 # s
 const t_outp =   10 * 86400  # s
 const t_chkp =   30 * 86400 # s
#const runTime = 24 * 3600 # s
#const t_outp =   1 * 3600  # s
#const t_chkp = runTime  # s
 const ntFrq_SC = 15 # frequency (in time-step) for State-Check output
#const runTime = 3 * 360 # s
#const t_outp = 3 * 360  # s
#const t_chkp = 0
