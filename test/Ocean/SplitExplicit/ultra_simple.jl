...
function main()
    ...
    model = OceanModel{FT}(prob, cʰ = cʰ, αᵀ = FT(0), fₒ = FT(0), β = FT(0) )
    ...
    lsrk_ocean = LSRKEulerMethod(dg, Q_3D, dt = dt_slow, t0 = 0)
    lsrk_horizontal =
        LSRKEulerMethod(horizontal_dg, Q_3D, dt = dt_slow, t0 = 0)
    lsrk_barotropic =
        LSRKEulerMethod(barotropic_dg, Q_2D, dt = dt_fast, t0 = 0)
